...
class TestPhysicsEngine(unittest.TestCase):
    def test_physics_object_creation(self):
        obj = PhysicsObject(mass=5.0, position=[1, 2, 3], velocity=[4, 5, 6], name="test")
    ...
    def test_gravity_between_objects(self):
        engine = ClassicalMechanicsEngine(gravitational_constant=1.0, softening_length=0.0)
    ...
    def test_energy_conservation(self):
        engine = ClassicalMechanicsEngine(gravitational_constant=1.0, softening_length=0.01)
        engine.add_object(PhysicsObject(mass=100.0, position=[0, 0, 0], name="s"))
        engine.add_object(PhysicsObject(mass=1.0, position=[10, 0, 0], velocity=[0, 1, 0], name="p"))
