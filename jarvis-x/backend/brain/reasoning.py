...
class ReasoningBrain:
    ...
    def _heuristic_plan(self, command: UserCommand, memory: list[dict]) -> Plan:
        ...
        if "list" in normalized and "file" in normalized:
            ...
            steps.append(
                ReasoningStep(order=1, title="Inspect directory", tool="list_files", input={"path": "."})
            )
            ...
            steps.append(
                ReasoningStep(order=1, title="Read file", tool="read_file", input={"path": target})
            )
        elif "status" in normalized or "metrics" in normalized:
            intent = "system_status"
            summary = "Retrieve latest telemetry and risk posture"
            steps.append(
                ReasoningStep(order=1, title="Get latest telemetry", tool="latest_events", input={})
            )
        else:
            summary = "Respond with a safe explanation and no tool execution"
            steps.append(ReasoningStep(order=1, title="Explain response", tool=None, input={}))
