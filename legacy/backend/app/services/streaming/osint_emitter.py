...
class OSINTFeedEmitter:
    ...
    async def start(self):
        self.running = True
        logger.info(
            "osint_emitter_started",
            simulation=self.settings.OSINT_STREAM_SIMULATION_ENABLED,
        )
    ...
    async def _emit_simulated_event(self):
        ...
        success = await self.producer.publish_event(
            self.settings.KAFKA_TOPIC_OSINT_STREAM, event, key=event["type"]
        )

        if success:
            logger.debug(
                "osint_event_emitted", event_id=event["event_id"], type=event["type"]
            )
        else:
