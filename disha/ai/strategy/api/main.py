...
class SimulationRequest(BaseModel):
    ...
    force_ratio: float = Field(
        1.0, ge=0.1, le=10.0, description="Attacker/defender force ratio"
    )
    technology_gap: float = Field(
        0.0, ge=-2.0, le=2.0, description="Technology advantage (-2 to +2)"
    )
    supply_lines: float = Field(
        0.8, ge=0.0, le=1.0, description="Attacker supply reliability (0-1)"
    )
...
async def list_conflicts(
    ...
    if strategy_type:
        results = [
            c
            for c in results
            if c.get("strategy_type", "").lower() == strategy_type.lower()
        ]
    if outcome:
        results = [
            c for c in results if c.get("outcome", "").lower() == outcome.lower()
        ]
    if terrain:
        results = [
            c for c in results if c.get("terrain", "").lower() == terrain.lower()
        ]
    if country:
...
async def get_stats():
    ...
    # Most casualties
    sorted_by_casualties = sorted(
        conflicts_db, key=lambda x: -x.get("casualties_estimate", 0)
    )
    deadliest_conflicts = [
        {
            "id": c["id"],
            "name": c["name"],
            "casualties_estimate": c.get("casualties_estimate"),
            "year": c.get("year"),
        }
...
async def list_strategies():
    ...
    strategy_info = {
        "Guerrilla": {
            ...
            "strengths": [
                "Effective in forests and mountains",
                "Low force requirements",
                "Hard to counter with conventional tactics",
            ],
            "weaknesses": [
                "Cannot hold territory easily",
                "Depends on civilian support",
                "Ineffective at sea",
            ],
            "famous_examples": [
                "Vietnam War",
        ...
        "Blitzkrieg": {
            ...
            "strengths": [
                "High speed and shock effect",
                "Devastating on open terrain",
                "Paralyzes enemy command",
            ],
            "weaknesses": [
                "Vulnerable to supply interdiction",
                "Ineffective in forests/mountains",
        ...
        "Conventional": {
            ...
            "strengths": [
                "Flexible",
                "Can be adapted to most situations",
                "Well-understood doctrine",
            ],
            "weaknesses": [
                "Predictable",
                "Can be countered by asymmetric tactics",
                "Resource intensive",
            ],
            "famous_examples": [
                "Battle of Waterloo",
        ...
        "Naval": {
            ...
            "strengths": [
                "Strategic reach",
                "Trade blockade capability",
                "Enables amphibious operations",
            ],
            "weaknesses": [
                "Limited to water environments",
                "Expensive and slow to rebuild losses",
        ...
        "Siege": {
            ...
            "strengths": [
                ...
                "Minimal maneuver required",
                "Can starve out defenders",
            ],
            "weaknesses": [
                "Slow",
                "Vulnerable to relief forces",
        ...
        "Attrition": {
            ...
            "strengths": [
                ...
                "Sustainable",
                "Degrades enemy over time",
            ],
            "weaknesses": [
                "Destroys own forces too",
                "Low morale impact",
                "Politically costly",
            ],
            "famous_examples": [
        ...
        "Flanking": {
            ...
            "strengths": [
                "Decisive when successful",
                "Maximizes kills per unit",
                "Psychological impact",
            ],
            "weaknesses": [
                "Requires mobility and coordination",
                "Flanks can be turned against you",
                "Complex execution",
            ],
            "famous_examples": [
                "Battle of Cannae",
        ...
        "Deception": {
            ...
            "strengths": [
                "Force multiplier",
                "Achieves surprise against larger forces",
                "Low cost",
            ],
            "weaknesses": [
                "Requires intelligence dominance",
                "Fails if penetrated",
                "One-time use per operation",
            ],
            "famous_examples": [
                "D-Day Deception (Fortitude)",
                "Battle of Midway",
                "Battle of Hastings feint",
            ],
        },
        "Psychological": {
            "description": "Targeting enemy morale, will to fight, and political cohesion rather than just military forces.",
            "strengths": [
                "Can win without military victory",
                "Amplified by media",
                "Exploits political divisions",
            ],
            "weaknesses": [
                "Unpredictable results",
                "Requires political context",
                "Can backfire",
            ],
            "famous_examples": [
                "Tet Offensive",
                "Mongol terror campaigns",
                "British strategic bombing WWII",
            ],
        },
        "Coalition": {
            "description": "Uniting multiple states or groups against a common enemy to multiply resources.",
            "strengths": [
                "Massive resource advantage",
...
async def run_simulation(request: SimulationRequest):
    ...
    if request.attacker_strategy not in valid_strategies:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid attacker_strategy. Must be one of: {valid_strategies}",
        )
    if request.defender_strategy not in valid_strategies:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid defender_strategy. Must be one of: {valid_strategies}",
        )
    valid_terrains = ["Plains", "Desert", "Mountains", "Forest", "Urban", "Sea"]
    if request.terrain not in valid_terrains:
        raise HTTPException(
            status_code=400, detail=f"Invalid terrain. Must be one of: {valid_terrains}"
        )
    valid_weather = ["Clear", "Rain", "Snow", "Storm"]
    if request.weather not in valid_weather:
        raise HTTPException(
            status_code=400, detail=f"Invalid weather. Must be one of: {valid_weather}"
        )
...
async def analyze_scenario(request: AnalysisRequest):
    ...
    if request.era:
        filtered = [
            c for c in filtered if c.get("era", "").lower() == request.era.lower()
        ]
    if request.region:
        filtered = [
            c for c in filtered if c.get("region", "").lower() == request.region.lower()
        ]
    if request.terrain:
        filtered = [
            c
            for c in filtered
            if c.get("terrain", "").lower() == request.terrain.lower()
        ]
        ...
        recommendations.append(
            {
                ...
                "historical_win_rate": round(sum(wins) / len(wins), 3)
                if wins
                else None,
                "historical_examples": len(wins),
                "terrain_effectiveness": STRATEGY_EFFECTIVENESS.get(strategy, {}).get(
                    terrain, 0.5
                ),
            }
        )
            ...
            result.append(
                {
                    ...
                    "avg_casualties": round(
                        sum(c.get("casualties_estimate", 0) for c in conflicts)
                        / data["total"],
                        0,
                    )
                    if data["total"] > 0
...
async def compare_strategies(request: CompareRequest):
    ...
    if request.strategy_a not in valid:
        raise HTTPException(
            status_code=400, detail=f"Invalid strategy_a. Must be one of: {valid}"
        )
    if request.strategy_b not in valid:
        raise HTTPException(
            status_code=400, detail=f"Invalid strategy_b. Must be one of: {valid}"
        )
