...
export function useCollaboration({
  ...
  useEffect(() => {
    ...
    cleanup.push(
      socket.on("session_state", (e: SessionStateEvent) => {
    ...
    cleanup.push(
      socket.on("tool_use_approved", (e: ToolUseApprovedEvent) => {
        setState((s) => ({
          ...s,
    ...
    cleanup.push(
      socket.on("tool_use_denied", (e: ToolUseDeniedEvent) => {
        setState((s) => ({
          ...s,
          pendingToolUses: s.pendingToolUses.filter((t) => t.id !== e.toolUseId),
        }));
    ...
    cleanup.push(
      socket.on("role_changed", (e: RoleChangedEvent) => {
    ...
    cleanup.push(
      socket.on("access_revoked", (e: AccessRevokedEvent) => {
        if (e.targetUserId === currentUser.id) {
          socket.disconnect();
    ...
    cleanup.push(
      socket.on("ownership_transferred", (e: OwnershipTransferredEvent) => {
    ...
    cleanup.push(
      socket.on("annotation_resolved", (e: AnnotationResolvedEvent) => {
        setState((s) => {
