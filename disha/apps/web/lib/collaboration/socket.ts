...
// Use a flexible record type since the union distribute-omit doesn't narrow correctly
export type OutgoingEvent = Record<string, unknown> & { type: CollabEventType };

// ── Event name → concrete type mapping ─────────────────────────────────────
...
export class CollabSocket {
  ...
  off<K extends CollabEventType>(type: K, handler: EventHandler<CollabEventMap[K]>): void {
  ...
  private scheduleReconnect(): void {
    ...
    const delay = this.baseDelay * 2 ** this.reconnectAttempts;
