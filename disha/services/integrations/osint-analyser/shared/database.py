...
class Database:
    ...
    def connect(self, username, password, db_name):
        try:
            ...
            uri = f"mysql+mysqlconnector://{username}:{password}@database/{db_name}"
            ...
            logging.error(
                "Non-SQLAlchemy error encountered when connecting "
    ...
    def get_content_attribute(self, content_id: int, attribute_name: str):
        with self.session_scope() as session:
            content = session.query(ContentModel).filter_by(id=content_id).first()
            if content is not None:
                return getattr(content, attribute_name, None)
            else:
                raise ContentNotFoundError(
                    f"No content record found with ID {content_id}"
                )

    # ----------------------------------------------------------------------- #

    def set_content_attribute(
        self, content_id: int, attribute_name: str, new_value
    ) -> None:
        with self.session_scope() as session:
            content = session.query(ContentModel).filter_by(id=content_id).first()
            if content is not None:
                if hasattr(content, attribute_name):
                    ...
                    session.commit()
                    logging.info(
                        f"Updated {attribute_name} for content ID {content_id}"
                    )
                else:
                    raise AttributeError(
                        f"Content record has no attribute '{attribute_name}'"
                    )
            else:
                raise ContentNotFoundError(
                    f"No content record found with ID {content_id}"
    ...
    def get_source_attribute(self, source_id: int, attribute_name: str):
        with self.session_scope() as session:
            source = session.query(SourceModel).filter_by(id=source_id).first()
            if source is not None:
                return getattr(source, attribute_name, None)
            else:
                raise ContentNotFoundError(
                    f"No source record found with ID {source_id}"
                )
    ...
    def get_analysis_requirements(self, source_id: int):
        ...
        with self.session_scope() as session:
            analysis_requirements = (
                session.query(AnalysisRequirementModel)
    ...
    def save_analysis_result(self, content_id: int, req_id: int, output: str):
        with self.session_scope() as session:
            new_analysis_result = AnalysisResultModel(
                req_id=req_id, content_id=content_id, output=output
            )
