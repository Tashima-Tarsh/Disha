...
class GPT:
    ...
    def execute(self):
        try:
            ...
            self.api_response = client.chat.completions.create(
                model=self.model.name, messages=self.messages
            )
        ...
        self.messages.append({"role": first_choice.message.role, "content": content})
    ...
    def _get_cost(self, api_response):
        input_cost = round(
            (api_response.usage.prompt_tokens / self.TOKENS_DIV_BY)
            * self.model.cost_input,
            self.COST_ROUNDING,
        )
        output_cost = round(
            (api_response.usage.completion_tokens / self.TOKENS_DIV_BY)
            * self.model.cost_output,
            self.COST_ROUNDING,
        )
