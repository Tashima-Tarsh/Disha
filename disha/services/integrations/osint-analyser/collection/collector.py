...
class Collector(ABC):
    ...
    def _register_collector(self) -> None:
        ...
        with self.database.session_scope() as session:
            ...
            if existing_collector is None:
                ...
                new_collector = CollectorModel(
                    short_name=self.short_name, long_name=self.long_name, enabled=True
                )
    ...
    def _get_collector_id(self) -> None:
        ...
        with self.database.session_scope() as session:
            ...
            if collector:
                ...
                logging.info(
                    f"Collector ID for '{self.short_name}' is {self.collector_id}."
                )
            else:
                logging.error(
                    f"Collector '{self.short_name}' not found in the database."
                )
    ...
    def add_source(self, uid: str, friendly_name: str) -> SourceModel | None:
        ...
        with self.database.session_scope() as session:
            ...
            if existing_source is None:
                ...
                logging.info(
                    f"Source '{uid}' already exists for collector '{self.short_name}'."
                )
        return None
    ...
    def add_content(
        self, source_uid: str, origin_time: datetime, text: str, metadata: dict
    ):
        """
