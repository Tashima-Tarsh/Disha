...
class Executor:
    ...
    def execute(self, plan: Plan) -> list[ExecutionResult]:
        ...
        for step in plan.steps:
            ...
            try:
                ...
                results.append(
                    ExecutionResult(success=True, step=step.title, output=output)
                )
            except Exception as exc:
                results.append(
                    ExecutionResult(
                        success=False, step=step.title, error=str(exc), output={}
                    )
                )
        return results
    ...
    def _run_tool(self, tool: str, tool_input: dict[str, Any]) -> dict[str, Any]:
        ...
        if tool == "read_file":
            path = self.policy.ensure_workspace_path(
                str(tool_input.get("path", "README.md"))
            )
            if not path.is_file():
        ...
        if tool == "latest_events":
            recent = self.store.recent_telemetry(
                device_id=tool_input.get("device_id", "desktop"), limit=5
            )
