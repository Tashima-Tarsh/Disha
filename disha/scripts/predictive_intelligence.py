...
class PredictiveAnalyzer:
    ...
    def analyze(self, days=90):
        ...
        for commit in Repository(self.repo_path, since=since).traverse_commits():
            is_fix = any(keyword in commit.msg.lower() for keyword in ['fix', 'bug', 'issue', 'repair', 'hotfix'])
            
            for m in commit.modified_files:
                filename = m.filename
                if filename not in file_stats:
                    file_stats[filename] = {'commits': 0, 'fixes': 0, 'authors': set(), 'complexity': m.complexity}
        ...
        for filename, stats in file_stats.items():
            ...
            risk_score = (stats['commits'] * 0.3) + (stats['fixes'] * 0.7) + (len(stats['authors']) * 0.2)
        ...
        logger.info("analysis_complete", high_risk_files=len([f for f in processed_stats if f['risk_score'] > 5.0]))
